...
export default function VoiceResult() {
  ...
  const acc = total > 0 ? correct / total : 0; // 正答率 0..1
  const comboTier = acc >= 0.9 ? 2 : acc >= 0.6 ? 1 : 0; // Riveの強度
  const trigger: "correct" | "wrong" = acc >= 0.6 ? "correct" : "wrong";

  // 画面を開くたびに一意のキーを持たせ、確実に1回発火
  const [fireKey] = useState(() => Date.now());
  ...
  }, [total, correct, acc, trigger, fireKey]);

  const title =
    acc >= 0.9 ? "いとめでたし！" : acc >= 0.6 ? "よきかな" : "むげなり";
  ...
  return (
    <div className="w-full max-w-xl p-6">
      ...
      {/* カニ：正答率に応じて反応＆強度 */}
      <Crab comboTier={comboTier} trigger={trigger} triggerKey={fireKey} />

      <div className="mt-6 grid gap-3">
        <Link
          to="/voice/session?mode=all10"
          className="block text-center px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500"
        >
          もう一度
        </Link>
        <Link
          to="/voice/session?mode=missed"
          className="block text-center px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500"
        >
          間違いだけで復習
        </Link>
        <Link
          to="/voice"
          className="block text-center px-4 py-2 rounded-lg bg-slate-600 hover:bg-slate-500"
        >
          メインに戻る
        </Link>
      </div>
